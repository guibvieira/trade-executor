...
.. code-block:: console
    ...
    export TRADING_STRATEGY_API_KEY="secret-token:tradingstrategy-a42e5d26fc1e3728fe5aa883c5d28d11cc76d2c0e965bd6346d4749232b0686c"
    trade-executor \
        backtest \
        ...
        trade-executor \
        backtest \
        --strategy-file=strategies/ethereum-memecoin-swing.py \
        --trading-strategy-api-key=$TRADING_STRATEGY_API_KEY
...
eth_parameters = StrategyParameters.from_class(Parameters)
...
def filter_pairs_by_risk(pairs_df: pd.DataFrame, 
                        risk_threshold: int = 60,
                        max_buy_tax: float = 6.0, 
                        max_sell_tax: float = 6.0,
                        risk_traits: dict = None) -> pd.DataFrame:
    ...
    logger.info(f"Initial pairs: {initial_count}")
    logger.info(f"After tax filters: {after_tax_count}")
    logger.info(f"After risk filters: {final_count}")
...
def prefilter_data(client: Client, execution_context) -> pd.DataFrame:
    ...
    ref_usdc_pair = pairs_df[pairs_df['address'] == "0x88e6a0c2ddd26feeb64f039a2c41296fcb3f5640"].copy()
    ...
    ref_usdc_pair = ref_usdc_pair[ref_usdc_pair['pair_id'].isin(passed_pair_ids)]
    pairs_df = pd.concat([pairs_df, ref_usdc_pair]).drop_duplicates(subset=['pair_id'])
...
def is_acceptable(
    ...
    if broken_trading_pairs is None:
        broken_trading_pairs = get_broken_pairs(
            indicators.strategy_universe, eth_parameters
        )
