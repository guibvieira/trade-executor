...
min_prefilter_liquidity = 10#1_000
...
print(f"we have liquidity data {len(liquidity_out_df.pair_id.unique())} pairs from {liquidity_out_df.timestamp.min()} to {liquidity_out_df.timestamp.max()}")
...
print(f"we have price data {len(price_df.pair_id.unique())} pairs from {price_df.timestamp.min()} to {price_df.timestamp.max()}")
